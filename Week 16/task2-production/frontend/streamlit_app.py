...
with st.sidebar:
    ...
    use_agentic = st.checkbox(
        "Enable Agentic Loop",
        value=False,
        help="Run cross-source verification with iterative tool calling",
    )
    if use_agentic:
        max_iterations = st.slider(
            ...
            max_value=5,
            value=5,
            help="Maximum model decisions before reporting incomplete verification",
        )
    else:
        max_iterations = 5
    ...
    if result:
        if use_agentic:
            ...
            st.session_state.messages.append({
                ...
                "tool_calls": [
                    {"tool": s.get("tool_name", ""), "result": s.get("tool_result_summary", "")}
            ...
            if steps:
                extras += '<div style="margin-top: 0.5rem; font-size: 0.85rem; color: #a0a0a0;">'
                extras += f"🔄 <strong>{result.get('iterations_used', 0)}</strong> iterations | "
                extras += f"⏱️ <strong>{result.get('duration_ms', 0):.0f}ms</strong>"
                if result.get("compacted"):
                    extras += " | 📦 Context compacted"
                extras += "</div>"

                # Show tools used
                tools_used = [s.get("tool_name") for s in steps if s.get("tool_name")]
                if tools_used:
                    extras += '<div style="margin-top: 0.3rem;">'
                    for tool in set(tools_used):
                        extras += f'<span class="source-badge">🔧 {tool}</span> '
                    extras += "</div>"
            ...
            if sources:
                ...
                extras += f'<div style="margin-top: 0.3rem">{sources_html}</div>'
            ...
            with col1:
                st.caption(f"🔄 Iterations: {result.get('iterations_used', 0)}")
            with col2:
                st.caption(f"📊 Tokens: {total_tok}")
            with col3:
                st.caption(f"⏱️ {elapsed:.2f}s")
            with col4:
                st.caption(f"🎯 {result.get('stopped_reason', 'N/A')}")

        else:
            # Standard response handling (existing code)
