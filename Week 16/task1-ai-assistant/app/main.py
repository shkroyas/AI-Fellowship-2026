...
def register_tools():
    ...
    # Register knowledge search tool (bound to RAG retriever if available)
    if rag_retriever:
        kb_tool = create_knowledge_search_tool(rag_retriever)
        tool_registry.register(
            name=kb_tool["name"],
            description=kb_tool["description"],
            parameters=kb_tool["parameters"],
            executor=kb_tool["executor"],
        )
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        # Register knowledge search tool now that RAG is ready
        kb_tool = create_knowledge_search_tool(rag_retriever)
        tool_registry.register(
            name=kb_tool["name"],
            description=kb_tool["description"],
...
class AgenticChatRequest(BaseModel):
    """Request for the agentic cross-source verification loop."""
    message: str = Field(..., description="User query to verify across sources")
    max_iterations: int = Field(default=5, ge=1, le=5, description="Max loop iterations")
    provider: Optional[str] = Field(default=None, description="LLM provider override")
...
async def agentic_chat(request: AgenticChatRequest):
    """
    Agentic cross-source verification endpoint.

    Runs an iterative loop where the LLM decides at each step whether to
    search the knowledge base, search the web, or provide a final answer.
    Uses context compaction to manage token budget across iterations.
    """
    try:
        ...
        # Convert steps to dicts for JSON serialization
        steps_dicts = []
        for step in result.steps:
            ...
            if step.tool_result_summary:
                step_dict["tool_result_summary"] = step.tool_result_summary
...
async def get_config():
    ...
    return {
        ...
        "agentic": {
            ...
            "compact_threshold": settings.agentic_compact_threshold,
        },
    }
